"""
bim_ui.charts — chart builders where the defect classes are structurally impossible.
...
def _label_text(values: Sequence[Any], fmt: NumberFormat | None) -> list[str]:
    if fmt is None:
        return [f"{v:,.0f}" if isinstance(v, (int, float)) else str(v) for v in values]
    return [fmt.render(v) for v in values]


def _collapse_tail(df: pd.DataFrame, cat: str, val: str, limit: int | None) -> pd.DataFrame:
    ...
    """
    if not limit or limit < 2 or df[cat].nunique() <= limit:
        return df
...
def bar_vs_target(df: pd.DataFrame, ctx, spec: ChartSpec) -> go.Figure | None:
    ...
    if df is None or len(df) == 0:
        return None
    p = ctx.palette
    d = df.sort_values(spec.x) if spec.x in df.columns else df

    fig = go.Figure()
    labels = _label_text(d[spec.y], spec.y_format) if spec.show_labels else None

    colors = p[ROLE_ACTUAL]
    if spec.color_by_status and spec.target and spec.target in d.columns:
        ratios = (d[spec.y] / d[spec.target].replace(0, pd.NA)).fillna(0)
...
def trend_line(df: pd.DataFrame, ctx, spec: ChartSpec,
    ...
    spec_no_labels = ChartSpec(**{**spec.__dict__, "show_labels": False})
...
def ranked_bar(df: pd.DataFrame, ctx, spec: ChartSpec) -> go.Figure | None:
    ...
    hspec = ChartSpec(**{**spec.__dict__, "x_format": spec.y_format})
...
def stacked_composition(df: pd.DataFrame, ctx, spec: ChartSpec) -> go.Figure | None:
    ...
    for cat in cats:
        ...
        grouped = sub.groupby(spec.x, as_index=False)[spec.y].sum()
        fig.add_trace(go.Bar(
    ...
    nolab = ChartSpec(**{**spec.__dict__, "show_labels": False})
...
def share_bar(df: pd.DataFrame, ctx, spec: ChartSpec) -> go.Figure | None:
    ...
    fig.update_layout(**layout)
    hspec = ChartSpec(**{**spec.__dict__, "x_format": PERCENT})
...
def cumulative_pace(df: pd.DataFrame, ctx, spec: ChartSpec, *,
    ...
    for cat, color in zip(cats, colors):
        sub = df[df[series] == cat].sort_values(spec.x)
...
def render(
    ...
    *,
    ...
    state, the same selection plumbing, and the same scroll-zoom config without
    each call site remembering to ask.
    """
    ...
    emits = spec.emits
    key = spec.key
    if emits and store is not None and key:
        store.register_emitter(key, emits)
        ...
        event = compat.plotly_chart(
            fig, on_select="rerun", use_container_width=True, key=key,
            selection_mode=("points", "box"), config=config,
        )
        if store.ingest_plotly(event, key, dimension=emits):
            if on_filter_change:
                on_filter_change()
            compat.rerun()
    else:
        compat.plotly_chart(fig, use_container_width=True, config=config, key=key)
