...
def cmd_report(args: argparse.Namespace) -> dict:
    ...
    verified_queries = generate_business_questions(
        ...
        model=getattr(args, 'questions_model', 'claude-sonnet-4-6'),
    )
...
def register_subparsers(subparsers) -> None:
    """Register the extraction subcommands onto an existing subparsers object.

    Split out of the original main() so the merged bi-modernization CLI can
    host these commands alongside its own without duplicating the argparse
    definitions. Note the merged CLI exposes `generate` as `generate-yaml`
    to avoid ambiguity with generate-streamlit / generate-react.
    """
    ...
    p_generate = subparsers.add_parser("generate-yaml", help="Generate Semantic View YAML.")
    ...
    p_report.add_argument("--questions-model", default="claude-sonnet-4-6",
                          help="Cortex LLM model for business question generation (default: claude-sonnet-4-6).")
...
# Command name -> handler. Consumed by the merged CLI dispatch.
COMMANDS = {
    "crawl": cmd_crawl,
    "parse": cmd_parse,
...
def main():
    """Standalone entry point, retained so this module still runs on its own."""
    parser = argparse.ArgumentParser(
        prog="python3 -m modules.cli_semex",
    ...
    log.info("extraction CLI invoked: %s", args.command)
